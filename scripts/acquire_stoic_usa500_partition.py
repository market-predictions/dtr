...
SYMBOL = "USA500IDXUSD"
OUTPUT_PREFIX = "usa500idxusd_m1_bid"
    ...
    for attempt in range(8):
        try:
            ...
            for offset in range(0, len(payload), RECORD.size):
                seconds, open_raw, close_raw, low_raw, high_raw, volume_raw = (
                    RECORD.unpack_from(payload, offset)
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Acquire a fixed Dukascopy USA500 M1 BID partition"
    )
    ...
    output = Path(f"{OUTPUT_PREFIX}_{args.label}.csv.gz")
    row_count = 0
    ...
    audit = {
        "classification": "Dukascopy USA500 bid-CFD proxy; not CME ES futures",
        "label": args.label,
    ...
    audit_path = Path(f"{OUTPUT_PREFIX}_{args.label}_audit.json")
