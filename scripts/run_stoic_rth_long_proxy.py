...
from stoic_123_lab import (
    load_config_family,
    rth_long_proxy_gates,
    rth_long_proxy_study,
    short_proxy_source,
)
...
def _validate_design(design: dict[str, object]) -> None:
    if str(design.get("status")) != "SOURCE_QUALIFIED_EXECUTION_AUTHORIZED":
        raise ValueError("RTH validation design is not execution authorized")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run frozen RTH long USATECH proxy validation"
    )
    ...
    history_frame, history_audits = short_proxy_source._load_partition(
        args.sources,
        design,
        "fresh_history",
    )
    holdout_frame, holdout_audits = short_proxy_source._load_partition(
        args.sources,
        design,
        "fresh_holdout",
    )

    history = rth_long_proxy_study.run_rth_partition(
    ...
    holdout = rth_long_proxy_study.run_rth_partition(
    ...
    gates_frame = pd.DataFrame(rth_long_proxy_gates.promotion_gates(history, holdout))
    ...
    if secondary_pass:
        status = "RETAIN_PRIMARY_AND_SECONDARY_FOR_ACTUAL_NQ_FUTURES_VALIDATION"
    elif primary_pass:
        status = "RETAIN_RTH_EMA_BREAK_FOR_ACTUAL_NQ_FUTURES_VALIDATION"
    else:
        status = "REJECT_OR_HOLD_NO_PROMOTION"
    ...
    decision = {
        ...
        "source_classification": rth_long_proxy_study.RTH_PROXY_SPEC.source_classification,
