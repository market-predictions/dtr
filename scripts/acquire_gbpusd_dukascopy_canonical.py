...
def fetch_day(day: dt.date, side: str) -> tuple[dt.date, list[tuple[object, ...]], str]:
    month = day.month - 1
    url = (
        f"https://datafeed.dukascopy.com/datafeed/{SYMBOL}/"
        f"{day.year:04d}/{month:02d}/{day.day:02d}/{side}_candles_min_1.bi5"
    )
...
def acquire_year(year: int, out_dir: Path, workers: int) -> dict[str, object]:
    ...
    audit: dict[str, object] = {
        ...
        "csv_field_order": ["timestamp", "open", "high", "low", "close", "volume"],
        "sides": {},
    }
    sides = audit["sides"]
    assert isinstance(sides, dict)
    for side in ("BID", "ASK"):
        results: dict[dt.date, list[tuple[object, ...]]] = {}
...
def main() -> None:
    ...
    path = args.out / f"gbpusd_{args.start_year}_{args.end_year}_manifest.json"
