...
def sanitize_sessions(
    sessions: pd.DataFrame, bars: pd.DataFrame, gaps: pd.DataFrame
) -> pd.DataFrame:
    reset = gaps.loc[
        gaps["reset_strategy_state"], ["previous_timestamp", "current_timestamp"]
    ].copy()
    ...
    for row in sessions.itertuples(index=False):
        ...
        if path_overlap.any():
            ...
            gap_bar = max(
                0, int(np.searchsorted(bar_times, np.datetime64(first_missing), side="right") - 1)
            )
            if gap_bar < post_end:
                post_end = gap_bar
                truncated = True
    ...
    for signal_id, signal in enumerate(signals):
        trade = engine._simulate_trade_np(
            *arrays,
            ...
            signal,
            cfg,
            unsafe_previous_ns=unsafe_prev,
            unsafe_current_ns=unsafe_curr,
...
def sequence(
    signal_features: pd.DataFrame, cached: dict[int, engine.Trade], mask: pd.Series
) -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    next_free = pd.Timestamp.min
    for row in (
        signal_features.loc[mask.fillna(False)].sort_values("entry_time").itertuples(index=False)
    ):
...
def rolling_prior_percentile(
    frame: pd.DataFrame, column: str, *, window: int = 126, minimum: int = 40
) -> pd.Series:
...
def daily_weekly(one: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    daily = (
        work.groupby("eth_day", as_index=False)
        .agg(
            open=("open", "first"),
            high=("high", "max"),
            low=("low", "min"),
    ...
    daily["week_start"] = daily["eth_day"] - pd.to_timedelta(
        (daily["eth_day"].dt.weekday + 1) % 7, unit="D"
    )
    weekly = (
...
def replay_path(
    signal: engine.CandidateSignal, bars: pd.DataFrame, cfg: engine.StrategyConfig
) -> dict[str, float | int]:
    ...
    for index in range(signal.sweep_index, reclaim_limit):
        if (signal.direction > 0 and close[index] >= signal.range_low) or (
            signal.direction < 0 and close[index] <= signal.range_high
        ):
            reclaim_index = index
            break
    if reclaim_index < 0:
    ...
    for index in range(max(signal.sweep_index + 2 * cfg.pivot_len, reclaim_index), last):
        ...
        if signal.direction > 0 and engine._pivot_high(high, candidate_index, cfg.pivot_len):
            if high[candidate_index] - signal.sweep_extreme >= max(
                cfg.tick_size, (signal.range_high - signal.range_low) * cfg.pivot_min_pct
            ):
            ...
            if signal.sweep_extreme - low[candidate_index] >= max(
                cfg.tick_size, (signal.range_high - signal.range_low) * cfg.pivot_min_pct
            ):
        ...
        if not np.isfinite(pivot) or index < pivot_ready:
            continue
        buffer = max(
            (signal.range_high - signal.range_low) * cfg.break_buffer_pct,
            atr[index] * cfg.break_atr_frac if np.isfinite(atr[index]) else 0.0,
        )
        broke = (
            high[index] >= pivot + buffer if signal.direction > 0 else low[index] <= pivot - buffer
        )
        if cfg.break_mode != "wick":
            broke = (
                close[index] >= pivot + buffer
                if signal.direction > 0
                else close[index] <= pivot - buffer
            )
        if not broke:
            continue
        if cfg.require_impulse:
            impulse = (
                np.isfinite(median_range[index])
                and (high[index] - low[index]) > median_range[index] * cfg.impulse_mult
                and (
    ...
    close_location = (
        (
            (close[bos_index] - low[bos_index]) / bos_range
            if signal.direction > 0
            else (high[bos_index] - close[bos_index]) / bos_range
        )
        if bos_range > 0
        else 0.0
    )
    ...
    atr_ratio = (
        true_range / atr[bos_index]
        if np.isfinite(atr[bos_index]) and atr[bos_index] > 0
        else np.nan
    )
    quality = (
        int(body_fraction >= 0.60)
        + int(close_location >= 0.75)
        + int(np.isfinite(atr_ratio) and atr_ratio >= 1.0)
    )
    return {
        "sweep_to_entry_bars": int(signal.entry_index - signal.sweep_index),
        "bos_quality_score": quality,
        "bos_body_fraction": float(body_fraction),
        "bos_close_location": float(close_location),
        "bos_atr_ratio": float(atr_ratio),
    }
...
def geometry(
    ...
    return {
        "entry_calc": float(entry),
        "stop_calc": float(stop),
        "risk_points": float(risk),
        "entry_extension_r": float(abs(signal.entry_price_raw - signal.pivot) / risk)
...
def nearest_clearance(row: pd.Series) -> float:
    ...
    if int(row["direction"]) > 0:
        ...
        distances = [
            float(value) - entry for value in candidates if pd.notna(value) and float(value) > entry
        ]
    else:
        candidates = [
            row["range_low"],
        ...
        distances = [
            entry - float(value) for value in candidates if pd.notna(value) and float(value) < entry
        ]
...
def add_features(
    ...
    session_context = pd.merge_asof(
        session_context,
        daily[["complete_time", "high", "low", "mid", "atr20"]].sort_values("complete_time"),
        left_on="range_start",
    ...
    session_context = pd.merge_asof(
        session_context.sort_values("range_start"),
        weekly[["complete_time", "high", "low", "mid"]].sort_values("complete_time"),
        left_on="range_start",
        right_on="complete_time",
        direction="backward",
        allow_exact_matches=True,
        suffixes=("", "_week"),
    ).rename(columns={"high": "prev_week_high", "low": "prev_week_low", "mid": "prev_week_mid"})
    keep = [
        "session_date",
        "session",
        "range_percentile",
    ...
    features = features.merge(
        session_context[keep], on=["session_date", "session"], how="left", validate="many_to_one"
    )
    features["directional_extreme_distance_atr"] = (
        np.where(
            features["direction"] > 0,
            (features["range_low"] - features["prev_d1_low"]).abs(),
            (features["range_high"] - features["prev_d1_high"]).abs(),
        )
        / features["d1_atr20"]
    )
...
def summarize(
    ...
    result["single_year_positive_net_share"] = (
        float(max(year_nets) / positive_total) if positive_total > 0 else np.inf
    )
    if not trades.empty:
        session_nets = trades.groupby("session")["pnl_r"].sum()
        result["single_session_positive_net_share"] = (
            float(session_nets.max() / positive_total) if positive_total > 0 else np.inf
        )
    else:
...
def block_bootstrap(
    trades: pd.DataFrame, unit: str, iterations: int, seed: int
) -> dict[str, float | int]:
    if trades.empty:
        return {
            "blocks": 0,
...
def paired_date_bootstrap(
    ...
    differences = candidate_series.reindex(index, fill_value=0.0) - baseline_series.reindex(
    ...
    return {
        "observed_net_difference_r": float(values.sum()),
        "lo95_net_difference_r": float(np.quantile(samples, 0.025)),
        "hi95_net_difference_r": float(np.quantile(samples, 0.975)),
        "prob_net_difference_positive": float(np.mean(samples > 0)),
    }


def eligible_count(
    sessions: pd.DataFrame, weekdays: tuple[int, ...], allowed_sessions: tuple[str, ...]
) -> int:
    return int(
        sessions.loc[
            sessions["weekday"].isin(weekdays) & sessions["session"].isin(allowed_sessions)
        ].shape[0]
    )
...
def changed_attribution(parent: pd.DataFrame, child: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    merged["change_type"] = merged["_merge"].map(
        {"left_only": "REMOVED", "right_only": "ADDED", "both": "UNCHANGED"}
    )
...
def gate_stage1(row: pd.Series, baseline: pd.Series) -> dict[str, bool]:
    return {
        ...
        "gate_effect": bool(
            row["return_dd"] >= baseline["return_dd"] * 1.15
            or (
                row["max_drawdown_r"] <= baseline["max_drawdown_r"] * 0.80
                and row["expectancy_r"] >= baseline["expectancy_r"]
            )
        ),
...
def event_gate(
    ...
    if removed.empty:
        ...
        negative = (
            -removed.loc[removed["pnl_r_parent"] < 0].groupby("entry_date")["pnl_r_parent"].sum()
        )
        date_diverse = bool(
            len(negative) >= 2
            and (negative.max() / negative.sum() <= 0.70 if negative.sum() > 0 else False)
        )
    return {
        ...
        "gate_cost": bool(
            summary["two_tick_expectancy_r"] >= parent_summary["two_tick_expectancy_r"]
        ),
...
def main() -> None:
    ...
    broad_cfg = replace(
        USA500_PROXY_SPEC.strategy_config(name="USA500_CORE_BROAD"),
        weekdays=(0, 1, 2, 3, 4),
    ...
    for index, (arm, (weekdays, allowed_sessions)) in enumerate(calendar_arms.items()):
        ...
        row = summarize(
            trades,
            arm,
            eligible_count(eligible, weekdays, allowed_sessions),
            int(mask.sum()),
            broad_cfg.tick_size,
        )
    ...
    for index, row in stage1.iterrows():
        ...
        stage1.loc[index, "uncertainty_class"] = (
            "SUPPORTED"
    ...
    selected_calendar = (
        ...
        else passing_stage1.sort_values(
    ...
    stage1.loc[stage1["arm"] == selected_calendar].iloc[0]

    context_masks = {
        "C0_CALENDAR_BASELINE": calendar_mask,
        "C1_EXCLUDE_COMPRESSED_RANGE": calendar_mask
        & (features["range_percentile"].isna() | (features["range_percentile"] >= 1 / 3)),
        "C2_EXCLUDE_NEAR_PRIOR_DAY_EXTREME": calendar_mask
        & (
    ...
    for index, (arm, mask) in enumerate(context_masks.items()):
        ...
        paired = (
            ...
            else paired_date_bootstrap(
                trades, selected_calendar_trades, args.iterations, args.seed + 200 + index
            )
        )
        row.update(paired)
        ...
        if arm != "C0_CALENDAR_BASELINE":
            context_changed.append(
                changed_attribution(selected_calendar_trades, trades, f"{selected_calendar}->{arm}")
            )
    context = pd.DataFrame(context_rows)
    baseline_context = context.loc[context["arm"] == "C0_CALENDAR_BASELINE"].iloc[0]
    for index, row in context.iterrows():
        flags = (
            gate_stage2(row, baseline_context)
            if row["arm"] != "C0_CALENDAR_BASELINE"
            else {
                key: True
                for key in (
                    "gate_retention",
        ...
        context.loc[index, "uncertainty_class"] = (
            "SUPPORTED"
            if row["lo95_net_difference_r"] > 0
            else "EXPLORATORY"
            if row["observed_net_difference_r"] > 0
    ...
    context.loc[context["arm"] == selected_context].iloc[0]

    events = event_masks(features)
    event_rows = []
    event_trades: dict[str, pd.DataFrame] = {"E0_CONTEXT_BASELINE": selected_context_trades}
    event_changes = []
    base_event_summary = summarize(
        selected_context_trades,
        "E0_CONTEXT_BASELINE",
        eligible_count(eligible, weekdays, allowed_sessions),
        int(selected_context_mask.sum()),
        broad_cfg.tick_size,
    )
    base_event_summary.update(
        {
            "observed_net_difference_r": 0.0,
            "lo95_net_difference_r": 0.0,
            "hi95_net_difference_r": 0.0,
    ...
    for index, (event_name, event_mask) in enumerate(events.items()):
        ...
        summary = summarize(
            ...
            arm,
            eligible_count(eligible, weekdays, allowed_sessions),
            int((selected_context_mask & event_mask).sum()),
            broad_cfg.tick_size,
        )
        summary.update(
            paired_date_bootstrap(
                trades, selected_context_trades, args.iterations, args.seed + 400 + index
            )
        )
    ...
    passing_events = events_frame.loc[
        (events_frame["arm"] != "E0_CONTEXT_BASELINE") & events_frame["gate_all"].fillna(False)
    ]
    selected_event = (
        "E0_CONTEXT_BASELINE"
    ...
    pd.concat(context_changed, ignore_index=True).to_csv(
        args.out / "stage2_changed_trade_attribution.csv", index=False
    )
    pd.concat(event_changes, ignore_index=True).to_csv(
        args.out / "stage3_changed_trade_attribution.csv", index=False
    )
    ...
    controls_path = (
        ...
        / "results"
        / "2026-07-22"
        / "nq_usa500_parallel_summary.csv"
    )
    controls = (
        pd.read_csv(controls_path).to_dict(orient="records") if controls_path.exists() else []
    )
    decision = {
        ...
        "selected_calendar_uncertainty": str(
            stage1.loc[stage1["arm"] == selected_calendar, "uncertainty_class"].iloc[0]
        ),
        "selected_context": selected_context,
        "selected_context_uncertainty": str(
            context.loc[context["arm"] == selected_context, "uncertainty_class"].iloc[0]
        ),
        "selected_event": selected_event,
        "final_candidate": selected_event
