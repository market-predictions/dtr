...
def _payload(dataset_path: str, digest: str) -> dict:
    return {
        ...
        "variants": [
            ...
            {"name": "recent", "policy": "recent_3"},
        ],
        "execution": {"gap_policy": "reject_unsafe"},
    }


def test_ifvg_manifest_loads_and_verifies_dataset(tmp_path: Path) -> None:
    ...
    manifest = load_ifvg_manifest(path)
    ...
    assert verify_ifvg_dataset(manifest, path) == dataset.resolve()


def test_ifvg_manifest_requires_exactly_one_observe_variant(tmp_path: Path) -> None:
    payload = _payload("sample.zip", "0" * 64)
    payload["variants"] = [{"name": "recent", "policy": "recent_3"}]
    path = tmp_path / "manifest.yaml"
    path.write_text(yaml.safe_dump(payload), encoding="utf-8")

    with pytest.raises(ValueError, match="Exactly one observe"):
        load_ifvg_manifest(path)


def test_ifvg_manifest_rejects_unknown_strategy_field(tmp_path: Path) -> None:
    payload = _payload("sample.zip", "0" * 64)
