...
def test_aggressive_reclaim_emits_signal_after_close() -> None:
    ...
    row = _london_row(build_event_ledger("NQ", _one_minute(), bars, _cfg(AsiaSweepVariant.AGGRESSIVE_RECLAIM)))
...
def test_wick_variant_requires_morphology() -> None:
    ...
    row = _london_row(build_event_ledger("NQ", _one_minute(), bars, _cfg(AsiaSweepVariant.WICK_QUALIFIED)))
    assert row["status"] == "REJECTED"
...
def test_displacement_uses_only_prior_body_reference() -> None:
    ...
    row = _london_row(build_event_ledger("NQ", _one_minute(), bars, _cfg(AsiaSweepVariant.DISPLACEMENT)))
    assert row["status"] == "SIGNAL"
...
def test_failed_retest_waits_for_confirmed_reaction_and_break() -> None:
    ...
    row = _london_row(build_event_ledger("NQ", _one_minute(), bars, _cfg(AsiaSweepVariant.FAILED_RETEST)))
    assert row["status"] == "SIGNAL"
