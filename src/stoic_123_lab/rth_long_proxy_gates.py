...
def promotion_gates(
    ...
    primary_checks = {
        ...
        "fresh_history_date_block_lo95_positive": (
            float(history["inferences"]["RTH_LONG_EMA_BREAK"]["lo95_expectancy_r"]) > 0
        ),
        "both_fresh_history_years_positive": positive_history_years == 2,
        "largest_positive_year_share_lte_0_65": (
            np.isfinite(float(history_break["largest_positive_year_share"]))
            and float(history_break["largest_positive_year_share"]) <= 0.65
        ),
        "fresh_history_two_tick_expectancy_positive": (
            float(history_summaries["RTH_LONG_EMA_BREAK_COST_2T"]["expectancy_r"]) > 0
        ),
        "fresh_history_one_minute_delay_expectancy_positive": (
            float(history_summaries["RTH_LONG_EMA_BREAK_DELAY_1M"]["expectancy_r"]) > 0
        ),
        ...
        "fresh_holdout_two_tick_expectancy_positive": (
            float(holdout_summaries["RTH_LONG_EMA_BREAK_COST_2T"]["expectancy_r"]) > 0
        ),
        "primary_matched_time_control_passes": (
            bool(_matched(history, "RTH_LONG_EMA_BREAK")["matched_control_passed"])
            and bool(_matched(holdout, "RTH_LONG_EMA_BREAK")["matched_control_passed"])
        ),
    }
    secondary_checks = {
