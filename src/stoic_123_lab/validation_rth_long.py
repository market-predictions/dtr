from __future__ import annotations

import hashlib
from dataclasses import dataclass
from datetime import time

import numpy as np
import pandas as pd

from .config import SequenceConfig
from .detector import attach_map
from .validation_direction import _event_row, _map_allows_long, _selected_map_direction

RTH_START = time(9, 30)
RTH_END = time(16, 0)
MINIMUM_MATCH_FRACTION = 0.90


@dataclass(frozen=True)
class _PreparedRthLongPools:
    bars: pd.DataFrame
    exact: dict[tuple[object, ...], np.ndarray]
    weekday: dict[tuple[object, ...], np.ndarray]
    broad: dict[tuple[object, ...], np.ndarray]


_POOL_CACHE: dict[
    tuple[int, int, SequenceConfig, str, str],
    _PreparedRthLongPools,
] = {}


def localize_session_time(
    timestamp: pd.Timestamp,
    *,
    source_timezone: str = "UTC",
    session_timezone: str = "America/New_York",
) -> pd.Timestamp:
    localized = pd.Timestamp(timestamp)
    if localized.tzinfo is None:
        localized = localized.tz_localize(source_timezone)
    else:
        localized = localized.tz_convert(source_timezone)
    return localized.tz_convert(session_timezone)


def session_label(
    timestamp: pd.Timestamp,
    *,
    source_timezone: str = "UTC",
    session_timezone: str = "America/New_York",
) -> str:
    clock = localize_session_time(
        timestamp,
        source_timezone=source_timezone,
        session_timezone=session_timezone,
    ).time()
    return "RTH" if RTH_START <= clock < RTH_END else "OVERNIGHT"


def filter_entry_events_by_session(
    events: pd.DataFrame,
    *,
    session: str,
    source_timezone: str = "UTC",
    session_timezone: str = "America/New_York",
) -> pd.DataFrame:
    if session not in {"RTH", "OVERNIGHT"}:
        raise ValueError(f"Unsupported session: {session}")
    if events.empty:
        return events.copy()
    if "signal_time" not in events:
        raise ValueError("Entry events require signal_time")
    labels = pd.to_datetime(events["signal_time"]).map(
        lambda value: session_label(
            value,
            source_timezone=source_timezone,
            session_timezone=session_timezone,
        )
    )
    result = events.loc[labels.eq(session)].copy()
    return result.sort_values("signal_time").reset_index(drop=True)


def _stable_offset(
    *,
    arm_id: str,
    signal_time: pd.Timestamp,
    seed: int,
    pool_size: int,
) -> int:
    payload = f"{arm_id}|rth-long|{signal_time.isoformat()}|{seed}".encode()
    return int.from_bytes(hashlib.sha256(payload).digest()[:8], "big") % pool_size


def _group_indices(
    bars: pd.DataFrame,
    columns: list[str],
) -> dict[tuple[object, ...], np.ndarray]:
    result: dict[tuple[object, ...], np.ndarray] = {}
    for key, group in bars.groupby(columns, sort=False, observed=True):
        normalized = key if isinstance(key, tuple) else (key,)
        result[normalized] = group.index.to_numpy(dtype=np.int64)
    return result


def _prepare_rth_long_pools(
    execution_bars: pd.DataFrame,
    map_bars: pd.DataFrame,
    config: SequenceConfig,
    *,
    source_timezone: str,
    session_timezone: str,
) -> _PreparedRthLongPools:
    key = (id(execution_bars), id(map_bars), config, source_timezone, session_timezone)
    cached = _POOL_CACHE.get(key)
    if cached is not None:
        return cached

    bars = attach_map(execution_bars, map_bars, config).reset_index(drop=True).copy()
    bars["signal_time"] = pd.to_datetime(bars["bar_end"])
    bars = bars.sort_values("signal_time").reset_index(drop=True)
    local = bars["signal_time"].map(
        lambda value: localize_session_time(
            value,
            source_timezone=source_timezone,
            session_timezone=session_timezone,
        )
    )
    bars["local_year"] = local.map(lambda value: value.year)
    bars["local_month"] = local.map(lambda value: value.month)
    bars["local_weekday"] = local.map(lambda value: value.weekday())
    bars["local_bucket"] = local.map(lambda value: value.hour * 2 + value.minute // 30)
    bars["session"] = local.map(
        lambda value: "RTH" if RTH_START <= value.time() < RTH_END else "OVERNIGHT"
    )
    bars["map_allows"] = [
        _map_allows_long(int(ema), int(breakout), config)
        for ema, breakout in zip(
            bars["ema_map_direction"].fillna(0),
            bars["breakout_map_direction"].fillna(0),
            strict=True,
        )
    ]

    eligible = np.isfinite(bars["atr"]) & bars["map_allows"] & bars["session"].eq("RTH")
    if "full_bar" in bars:
        eligible &= bars["full_bar"].fillna(False).astype(bool)
    if "gap_minutes" in bars:
        eligible &= bars["gap_minutes"].fillna(0).le(config.gap_reset_minutes)
    bars = bars.loc[eligible].reset_index(drop=True)

    prepared = _PreparedRthLongPools(
        bars=bars,
        exact=_group_indices(
            bars,
            ["local_year", "local_month", "local_weekday", "local_bucket"],
        ),
        weekday=_group_indices(
            bars,
            ["local_year", "local_weekday", "local_bucket"],
        ),
        broad=_group_indices(
            bars,
            ["local_year", "local_bucket"],
        ),
    )
    _POOL_CACHE.clear()
    _POOL_CACHE[key] = prepared
    return prepared


def matched_time_rth_long_events(
    full_events: pd.DataFrame,
    execution_bars: pd.DataFrame,
    map_bars: pd.DataFrame,
    config: SequenceConfig,
    *,
    seed: int,
    source_timezone: str = "UTC",
    session_timezone: str = "America/New_York",
) -> pd.DataFrame:
    """Build deterministic RTH long pseudo entries without using future returns."""

    if full_events.empty:
        result = pd.DataFrame()
        result.attrs["match_fraction"] = 0.0
        return result

    rth_events = filter_entry_events_by_session(
        full_events,
        session="RTH",
        source_timezone=source_timezone,
        session_timezone=session_timezone,
    )
    if rth_events.empty:
        result = pd.DataFrame()
        result.attrs["match_fraction"] = 0.0
        return result

    prepared = _prepare_rth_long_pools(
        execution_bars,
        map_bars,
        config,
        source_timezone=source_timezone,
        session_timezone=session_timezone,
    )
    bars = prepared.bars
    original_times = set(pd.to_datetime(rth_events["signal_time"]))
    used: set[pd.Timestamp] = set()
    rows: list[dict[str, object]] = []

    for event in rth_events.sort_values("signal_time").itertuples(index=False):
        original = pd.Timestamp(event.signal_time)
        risk_width = float(event.breakout_close) - float(event.protective_boundary)
        if not np.isfinite(risk_width) or risk_width <= 0:
            continue

        local = localize_session_time(
            original,
            source_timezone=source_timezone,
            session_timezone=session_timezone,
        )
        bucket = local.hour * 2 + local.minute // 30
        candidate_pools = (
            prepared.exact.get((local.year, local.month, local.weekday(), bucket)),
            prepared.weekday.get((local.year, local.weekday(), bucket)),
            prepared.broad.get((local.year, bucket)),
        )
        pool = next(
            (
                candidate
                for candidate in candidate_pools
                if candidate is not None and len(candidate) >= 2
            ),
            None,
        )
        if pool is None:
            continue

        start = _stable_offset(
            arm_id=config.arm_id,
            signal_time=original,
            seed=seed,
            pool_size=len(pool),
        )
        selected = None
        for step in range(len(pool)):
            candidate = bars.iloc[int(pool[(start + step) % len(pool)])]
            candidate_time = pd.Timestamp(candidate["signal_time"])
            if candidate_time in used or candidate_time in original_times:
                continue
            if abs((candidate_time - original).total_seconds()) < 24 * 60 * 60:
                continue
            selected = candidate
            break
        if selected is None:
            continue

        signal_time = pd.Timestamp(selected["signal_time"])
        used.add(signal_time)
        close_price = float(selected["close"])
        selected_map = _selected_map_direction(
            int(selected["ema_map_direction"]),
            int(selected["breakout_map_direction"]),
            config,
        )
        rows.append(
            _event_row(
                arm_id=f"{config.arm_id}__MATCHED_RTH_LONG",
                model="matched_time",
                step1_time=signal_time,
                retest_time=None,
                signal_time=signal_time,
                base_lock_time=pd.Timestamp(selected["timestamp"]),
                protective_boundary=close_price - risk_width,
                breakout_close=close_price,
                atr_at_signal=float(selected["atr"]),
                map_direction_step1=selected_map,
                map_direction_signal=selected_map,
            )
        )

    result = pd.DataFrame(rows)
    if not result.empty:
        result = result.sort_values("signal_time").reset_index(drop=True)
    result.attrs["match_fraction"] = len(result) / len(rth_events)
    return result
