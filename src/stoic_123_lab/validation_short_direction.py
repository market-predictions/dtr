...
from .validation_direction import StageDetection, _selected_map_direction

ShortStageModel = Literal["ema_break", "ema_break_retest"]


def _map_allows_short(
    ema_direction: int,
...
def _short_step1(
    ...
    *,
    ...
    crossed = close[index - 1] >= previous_zone_low and close[index] < zone_low - buffer
    return bool(
        crossed
        and _map_allows_short(
            int(ema_map[index]),
...
def _short_event_row(
    *,
    arm_id: str,
    model: ShortStageModel,
...
def detect_short_stage_events(
    ...
    *,
    ...
    """Detect causal short controls using the frozen Step-1 and retest rules."""
    ...
    for index in range(1, len(bars)):
        ...
        zone_low = min(ema_fast[index], ema_slow[index])
        retest = high[index] >= zone_low - tolerance and close[index] <= zone_high + tolerance
        if not retest:
            continue
        ...
        if config.require_map_at_step3 and not _map_allows_short(
            int(ema_map[index]),
            int(breakout_map[index]),
            config,
        ):
        ...
        events.append(
            _short_event_row(
                arm_id=f"{config.arm_id}__SHORT_EMA_BREAK_RETEST",
                model="ema_break_retest",
                step1_time=step1_timestamp,
                retest_time=signal_time,
                signal_time=signal_time,
                base_lock_time=step1_timestamp,
                protective_boundary=float(np.max(high[step1_index : index + 1])),
                breakout_close=float(close[index]),
                atr_at_signal=float(atr[index]),
                map_direction_step1=step1_map,
