...
def evaluate_final_holdout(
    ...
    validate_holdout_events(events)
    ...
    if years != HOLDOUT_YEARS or pairs != PAIRS:
        raise ValueError("T5 holdout population lost a required pair or year")
...
def write_final_holdout_outputs(
    output_dir: Path,
    result: dict[str, Any],
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    result["ledger"].to_csv(output_dir / "holdout_predictions.csv", index=False)
    result["weekly"].to_csv(output_dir / "holdout_weekly_ranking.csv", index=False)
    result["pair_table"].to_csv(output_dir / "holdout_pair_lift.csv", index=False)
    result["year_table"].to_csv(output_dir / "holdout_year_lift.csv", index=False)
    result["side_table"].to_csv(output_dir / "holdout_side_lift.csv", index=False)
    result["weekday_table"].to_csv(
        output_dir / "holdout_weekday_lift.csv", index=False
    )
    result["fingerprints"].to_csv(
        output_dir / "holdout_fingerprint_directions.csv", index=False
    )
    result["calibration"].to_csv(
        output_dir / "holdout_calibration.csv", index=False
    )
    payload = {
        "programme": "ASIAN_SWEEP_FINGERPRINT_V1",
        "stage": "UNTOUCHED_2024_2025_FINAL_HOLDOUT",
        ...
        "holdout_2024_2025_opened": True,
        "strategy_pnl_calculated": False,
    }
    (output_dir / "final_holdout_decision.json").write_text(
