...
def prepare_triage_ledger(ledger: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = frame.sort_values(["landmark", "event_id"], kind="mergesort").reset_index(
        drop=True
    )
...
def _component_fit_rows(frame: pd.DataFrame, target: str) -> pd.DataFrame:
    if target == "resolved_target":
        return frame
    if target == "reversal_target":
        return frame.loc[frame["resolved_target"].eq(1)]
    raise ValueError(target)


def _score_mask(frame: pd.DataFrame, target: str) -> np.ndarray:
    if target == "resolved_target":
        return np.ones(len(frame), dtype=bool)
    return frame["resolved_target"].eq(1).to_numpy()
...
def _fit_weighted_model(
    ...
    *,
    family: str,
    config: tuple[float, float | int],
    target: str,
):
    fit = _component_fit_rows(frame, target)
    if fit[target].nunique() != 2:
        raise ValueError(f"{target}: fit population does not contain two classes")
...
def _inner_component_predictions(
    train: pd.DataFrame,
    *,
    family: str,
    target: str,
    ...
    probabilities = np.full(len(train), math.nan, dtype=float)
    groups = train["week_key"].astype(str)
    folds = GroupKFold(n_splits=3)
    for fit_index, validation_index in folds.split(train, train[target], groups):
        fit = train.iloc[fit_index]
        validation = train.iloc[validation_index]
        model = _fit_weighted_model(
            fit, family=family, config=config, target=target
        )
        probabilities[validation_index] = model.predict_proba(validation)[:, 1]
    if not np.isfinite(probabilities).all():
        raise RuntimeError(f"{family}/{target}: incomplete inner predictions")
    score = _score_mask(train, target)
    clipped = np.clip(probabilities[score], 1e-6, 1.0 - 1e-6)
    truth = train.loc[score, target]
    return (
        probabilities,
        float(log_loss(truth, clipped)),
        float(brier_score_loss(truth, probabilities[score])),
    )
...
def _select_component(
    ...
    *,
    ...
    for config in grid:
        probabilities, loss, brier = _inner_component_predictions(
            train, family=family, target=target, config=config
        )
        rows.append(
            {
                "component": target,
        ...
        candidate = (loss, brier, config, probabilities)
        if best is None or candidate[:2] < best[:2]:
            best = candidate
    if best is None:
        raise RuntimeError(f"{family}/{target}: no selected configuration")
...
def _calibrated_component(
    ...
    *,
    ...
    config, inner_raw, tuning = _select_component(
        train, family=family, target=target
    )
    calibration_mask = _score_mask(train, target)
    calibrator = _fit_platt(
        train.loc[calibration_mask, target].to_numpy(),
        inner_raw[calibration_mask],
    )
    inner_probability = _apply_platt(calibrator, inner_raw)
    model = _fit_weighted_model(
        train, family=family, config=config, target=target
    )
    test_probability = _apply_platt(
        calibrator, model.predict_proba(test)[:, 1]
    )
    return inner_probability, test_probability, tuning
...
def probabilities_from_components(
    ...
    resolved = np.clip(p_resolved, 0.0, 1.0)
    conditional_reversal = np.clip(p_reversal_given_resolved, 0.0, 1.0)
    reversal = resolved * conditional_reversal
    continuation = resolved * (1.0 - conditional_reversal)
    unresolved = 1.0 - resolved
    total = reversal + continuation + unresolved
    return pd.DataFrame(
        {
            "p_continuation": continuation / total,
            "p_reversal": reversal / total,
            "p_unresolved": unresolved / total,
        }
    )
...
def action_metrics(outcome: np.ndarray, action: np.ndarray) -> dict[str, float]:
    ...
    reversal_call = action == "REVERSAL"
    continuation_call = action == "CONTINUATION"
    true_reversal = outcome == "REVERSAL"
    return {
        "coverage": float(take.mean()),
        "continuation_coverage": float(continuation_call.mean()),
        "reversal_calls": float(reversal_call.sum()),
        "continuation_precision": (
            float(correct[continuation_call].mean()) if continuation_call.any() else 0.0
        ),
        "reversal_precision": (
            float(correct[reversal_call].mean()) if reversal_call.any() else 0.0
        ),
        ...
        "continuation_false_veto_rate": (
            float((continuation_call & true_reversal).sum() / true_reversal.sum())
...
def _fold_baseline(train: pd.DataFrame, size: int) -> pd.DataFrame:
    frequency = train["triage_outcome"].value_counts(normalize=True)
    return pd.DataFrame(
        {
            "baseline_p_continuation": np.repeat(
                float(frequency.get("CONTINUATION", 0.0)), size
            ),
            "baseline_p_reversal": np.repeat(
                float(frequency.get("REVERSAL", 0.0)), size
            ),
            "baseline_p_unresolved": np.repeat(
                float(frequency.get("UNRESOLVED", 0.0)), size
            ),
        }
    )
...
def fit_outer_fold(
    ...
    *,
    ...
    resolved_inner, resolved_test, resolved_tuning = _calibrated_component(
        train, test, family=family, target="resolved_target"
    )
    direction_inner, direction_test, direction_tuning = _calibrated_component(
        train, test, family=family, target="reversal_target"
    )
    inner_probabilities = probabilities_from_components(
        resolved_inner, direction_inner
    )
    threshold, threshold_table = select_threshold(
        train["triage_outcome"].to_numpy(), inner_probabilities
    )
    ...
    output = pd.concat(
        [
            output,
            test_probabilities.reset_index(drop=True),
            _fold_baseline(train, len(test)),
        ],
        axis=1,
    )
    ...
    threshold_table = threshold_table.assign(
        component="action_threshold", family=family
    )
    return output, pd.concat(
        [threshold_table, tuning], ignore_index=True, sort=False
    )


def _multiclass_scores(predictions: pd.DataFrame) -> dict[str, float]:
    category = pd.Categorical(predictions["triage_outcome"], categories=CLASSES)
    ...
    baseline = predictions[
        [
            "baseline_p_continuation",
            "baseline_p_reversal",
            "baseline_p_unresolved",
        ]
    ].to_numpy()
    one_hot = np.eye(3)[target]
    return {
        "log_loss": float(log_loss(target, probability, labels=[0, 1, 2])),
        "baseline_log_loss": float(log_loss(target, baseline, labels=[0, 1, 2])),
        "multiclass_brier": float(
            np.mean(np.sum((one_hot - probability) ** 2, axis=1))
        ),
        "baseline_multiclass_brier": float(
            np.mean(np.sum((one_hot - baseline) ** 2, axis=1))
        ),
    }


def summarize_predictions(predictions: pd.DataFrame) -> dict[str, Any]:
    outcome = predictions["triage_outcome"].to_numpy()
    action = predictions["action"].to_numpy()
    result = {
        "events": int(len(predictions)),
        "resolved_events": int(predictions["triage_outcome"].ne("UNRESOLVED").sum()),
        **action_metrics(outcome, action),
    ...
    evaluated["utility"] = np.where(~take, 0.0, np.where(correct, 1.0, -1.5))
    pair_utility = evaluated.groupby("instrument")["utility"].mean()
    year_utility = evaluated.groupby("year")["utility"].mean()
    resolved_per_pair = evaluated.loc[
        evaluated["triage_outcome"].ne("UNRESOLVED")
    ...
    if not correct_reversals.empty:
        concentration = max(
            float(correct_reversals["instrument"].value_counts(normalize=True).max()),
            float(correct_reversals["side"].value_counts(normalize=True).max()),
            float(correct_reversals["weekday"].value_counts(normalize=True).max()),
        )
    result.update(
        {
            "minimum_pair_resolved_events": int(resolved_per_pair.min()),
            ...
            "median_reversal_call_reward_risk": float(
                evaluated.loc[evaluated["action"].eq("REVERSAL"), "reward_risk"].median()
            ),
        }
    )
    full_gates = {
        "resolved_events": result["resolved_events"] >= 1500,
        "resolved_events_per_pair": result["minimum_pair_resolved_events"] >= 600,
        ...
        "brier": result["multiclass_brier"] < result["baseline_multiclass_brier"],
...
def run_development(ledger: pd.DataFrame, output_directory: Path) -> dict[str, Any]:
    ...
    for landmark in LANDMARKS:
        ...
        for family in FAMILIES:
            ...
            for outer_year in DEVELOPMENT_YEARS:
                train = population.loc[population["year"].ne(outer_year)].reset_index(
                    drop=True
                )
                test = population.loc[population["year"].eq(outer_year)].reset_index(
                    drop=True
                )
                predictions, tuning = fit_outer_fold(train, test, family=family)
    ...
    candidates.to_csv(output_directory / "triage_candidate_summary.csv", index=False)
    ...
    decision.update(
        {
            "programme": "ASIAN_SWEEP_TRIAGE_V6_4_CAUSAL",
