...
def _policy_complexity(policy: pd.Series | dict[str, Any]) -> tuple[int, int, int]:
    target = str(policy["target_kind"])
    hour = int(policy["exit_hour"])
    stop = str(policy["stop_policy"])
    return (
        TARGET_KINDS.index(target),
        EXIT_HOURS.index(hour),
        STOP_POLICIES.index(stop),
    )
...
def summarize_trades(trades: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        **calendar_week_bootstrap(trades),
...
def _inner_policy_table(grid: pd.DataFrame, outer_year: int) -> pd.DataFrame:
    ...
    for policy_id, trades in inner.groupby("policy_id", sort=True):
        summary = summarize_trades(trades)
        pair_table = summary["pair_table"]
        year_table = summary["year_table"]
        ...
        rows.append(
            {
                ...
                "positive_pairs": int((pair_table["expectancy"] > 0).sum()),
                "positive_inner_years": int((year_table["expectancy"] > 0).sum()),
            }
        )
...
def build_nested_runner_decision(grid: pd.DataFrame) -> dict[str, Any]:
    ...
    if sorted(grid["policy_id"].unique()) != sorted(
        row["policy_id"] for row in runner_policy_manifest()
    ):
    ...
    for outer_year in years:
        ...
        heldout = grid.loc[
            grid["year"].eq(outer_year) & grid["policy_id"].eq(policy_id)
        ].copy()
        heldout_summary = summarize_trades(heldout)
    ...
    selected_ids = [
        value for value in fold_table["selected_policy_id"].dropna().astype(str).tolist()
    ]
    ...
    oof_summary = summarize_trades(oof) if not oof.empty else summarize_trades(grid.iloc[0:0])
    modal_trades = (
    ...
    gates = {
        "all_outer_folds_selected": bool(len(selected_ids) == 5),
        "at_least_150_oof_trades": oof_summary["trade_count"] >= 150,
        "at_least_50_oof_trades_per_pair": oof_summary["minimum_pair_trades"] >= 50,
        "oof_expectancy_above_0_03r": bool(oof_summary["expectancy"] > 0.03),
        "oof_positive_under_0_10_pip_stress": bool(
            oof_summary["stress_0_10_expectancy"] > 0
        ),
        "oof_both_pairs_positive": bool(
            len(oof_pair) == 2 and (oof_pair["expectancy"] > 0).all()
        ),
        "oof_four_of_five_years_positive": bool(
            len(oof_year) == 5 and (oof_year["expectancy"] > 0).sum() >= 4
        ),
        "oof_median_annual_expectancy_positive": bool(
            oof_summary["median_annual_expectancy"] > 0
        ),
        "oof_max_drawdown_below_25r": bool(oof_summary["max_drawdown_r"] < 25),
        "oof_return_drawdown_at_least_1_50": bool(
            oof_summary["return_drawdown"] >= 1.50
        ),
        "oof_bootstrap_probability_positive_at_least_95pct": bool(
            oof_summary["probability_expectancy_positive"] >= 0.95
        ),
        "modal_policy_selected_at_least_three_folds": modal_count >= 3,
        "oof_largest_winner_below_20pct_net": bool(
            np.isfinite(oof_summary["largest_winner_share_of_net_r"])
            and oof_summary["largest_winner_share_of_net_r"] <= 0.20
        ),
        "modal_policy_base_expectancy_positive": bool(modal_summary["expectancy"] > 0),
        "modal_policy_stress_expectancy_positive": bool(
            modal_summary["stress_0_10_expectancy"] > 0
        ),
        "modal_policy_both_pairs_positive": bool(
            len(modal_pair) == 2 and (modal_pair["expectancy"] > 0).all()
        ),
        "modal_policy_four_of_five_years_positive": bool(
            len(modal_year) == 5 and (modal_year["expectancy"] > 0).sum() >= 4
        ),
    }
...
def write_runner_discovery_outputs(output_directory: Path, grid: pd.DataFrame) -> dict[str, Any]:
    ...
    for policy, trades in grid.groupby("policy_id", sort=True):
        summary = summarize_trades(trades)
        row = trades.iloc[0]
        policy_rows.append(
            {
