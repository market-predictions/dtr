...
def _candidate_population(
    ...
    *,
    ...
    return frame.sort_values(["year", "event_id"], kind="mergesort").reset_index(
        drop=True
    )
...
def _fit_weighted(
    ...
    *,
    ...
    if frame["model_target"].nunique() != 2:
        raise ValueError("extended target fit population does not contain two classes")
    model = _make_model(family, config)
    weights = compute_sample_weight("balanced", frame["model_target"])
    model.fit(frame, frame["model_target"], model__sample_weight=weights)
    return model
...
def _fit_outer_predictions(
    ...
    *,
    ...
    for outer_year in DEVELOPMENT_YEARS:
        ...
        for config in grid:
            probability = np.full(len(train), math.nan, dtype=float)
            for fit_index, validation_index in folds.split(
                train, train["model_target"], groups
            ):
                model = _fit_weighted(
                    train.iloc[fit_index], family=family, config=config
                )
            ...
            loss = float(
                log_loss(train["model_target"], clipped, labels=[0, 1])
            )
            pr_auc = float(
                average_precision_score(train["model_target"], clipped)
            )
            candidate = (loss, -pr_auc, config, probability)
            if best is None or candidate[:2] < best[:2]:
                best = candidate
        if best is None:
            raise RuntimeError(f"{landmark}/{target_name}/{family}: no fitted model")
        loss, negative_pr_auc, config, inner_probability = best
        calibrator = _fit_platt(
            train["model_target"].to_numpy(), inner_probability
        )
        model = _fit_weighted(train, family=family, config=config)
...
def summarize_extended_predictions(predictions: pd.DataFrame) -> dict[str, Any]:
    ...
    stressed_reward = reward - (2.0 if target_name == "LATE_TREND_HOLD_1600" else 1.0) * slippage
    stressed_loss = 1.0 + 2.0 * slippage
    expected_value = (
        top["probability"] * stressed_reward
        - (1.0 - top["probability"]) * stressed_loss
    )
    pair = frame.groupby("instrument").agg(
        events=("model_target", "size"),
        positives=("model_target", "sum"),
        base_rate=("model_target", "mean"),
    )
    pair_top = frame.loc[frame["quintile"].eq(5)].groupby("instrument")[
        "model_target"
    ].mean()
    pair["top_rate"] = pair_top
    pair["positive_lift"] = pair["top_rate"] > pair["base_rate"]
    year = frame.groupby("year").agg(
        events=("model_target", "size"),
        base_rate=("model_target", "mean"),
    )
    year_top = frame.loc[frame["quintile"].eq(5)].groupby("year")[
        "model_target"
    ].mean()
    year["top_rate"] = year_top
    year["positive_lift"] = year["top_rate"] > year["base_rate"]
    ...
    summary = {
        ...
        "year_positive_lift_count": int(year["positive_lift"].sum()),
...
def fit_extended_candidate(
    ...
    *,
    ...
    summary.update({"landmark": landmark, "target_name": target, "family": family})
    output_directory.mkdir(parents=True, exist_ok=True)
    stem = f"{landmark}_{target}_{family}"
    predictions.to_csv(
        output_directory / f"extended_predictions_{stem}.csv", index=False
    )
    tuning.to_csv(output_directory / f"extended_tuning_{stem}.csv", index=False)
...
def aggregate_extended_summaries(
    ...
    expected = len(LANDMARKS) * len(TARGETS) * len(FAMILIES)
    if len(rows) != expected:
        raise ValueError(f"expected {expected} extended summaries, found {len(rows)}")
    ...
    if passing.empty:
        ...
        passing["target_order"] = passing["target_name"].map(
            {
                "EXTENDED_3R_1600": 3.0,
                "EXTENDED_2R_1300": 2.0,
                "LATE_TREND_HOLD_1600": 2.0,
