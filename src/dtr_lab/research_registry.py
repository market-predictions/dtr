...
CANONICAL_PAIRS = frozenset({"EURUSD", "GBPUSD", "USDCHF", "AUDUSD", "NZDUSD", "USDCAD", "USDJPY", "EURJPY", "GBPJPY", "EURGBP"})
STUDY_ID_RE = re.compile(r"^DFXC-\d{8}-\d{3}-[a-z0-9-]+$")
SHA_RE = re.compile(r"^[0-9a-f]{40}$")
LIFECYCLE = {"DESIGN", "PREREGISTERED", "RUN_COMPLETE", "INTERNAL_ASSURANCE_PASS", "HOLDOUT_CONFIRMED", "REJECTED", "SUPERSEDED", "ABANDONED"}
PRE_RESULT_LIFECYCLE = {"DESIGN", "PREREGISTERED"}
DISPOSITIONS = {"SUPPORTED_INTERNAL", "PROMOTE_TO_HOLDOUT_CONFIRMATION", "REJECT_NO_INCREMENTAL_VALUE", "REJECT_MECHANISM", "DESCRIPTIVE_ONLY", "HOLD_FOR_FRESH_DATA", "INDETERMINATE", "INVALIDATED_DATA", "SUPERSEDED", "ABANDONED"}
HOLDOUT_STATES = {"UNOPENED", "PARTIALLY_OPENED", "OPENED", "NOT_APPLICABLE"}
...
def validate_study(study: dict[str, Any]) -> list[str]:
    ...
    if not study.get("hypotheses", {}).get("market_question"):
        errors.append(f"{sid}: market_question is required")
    if not study.get("hypotheses", {}).get("null"):
        errors.append(f"{sid}: null hypothesis is required")
    if not study.get("hypotheses", {}).get("alternative"):
        errors.append(f"{sid}: alternative hypothesis is required")
    if not study.get("method", {}).get("primary_endpoint"):
        errors.append(f"{sid}: primary_endpoint is required")
    if not study.get("method", {}).get("controls"):
        errors.append(f"{sid}: at least one baseline/control or an explicit no-control rationale is required")
...
def validate_result(result: dict[str, Any], study: dict[str, Any]) -> list[str]:
    ...
    if bool(holdout.get("opened_in_this_study")) != bool(study.get("holdout", {}).get("opened_in_this_study")):
...
def render_registry_markdown(index: dict[str, Any]) -> str:
    studies = sorted(index["studies"], key=lambda entry: (entry["date"], entry["study_id"]))
    lines = [
    ...
    for entry in studies:
        ...
        lines.append(f"| `{entry['study_id']}` | {entry['date']} | {entry['family']} | {entry['title']} | `{disposition}` | `{entry['holdout_state']}` | `{entry['assurance_status']}` | {source} |")
    lines.extend([
        "",
        "## Reading rule",
        "",
...
def validate_registry(repo_root: Path) -> list[str]:
    ...
    if dataset.get("canonical_name") != DATASET_NAME or dataset.get("dataset_id") != DATASET_ID:
    ...
    expected_order = sorted(entries, key=lambda entry: (entry.get("date", ""), entry.get("study_id", "")))
    ...
    for entry in entries:
        ...
        if not result_path_value:
            ...
            if not result_path.exists():
                ...
                if result.get("scientific_disposition") != entry.get("scientific_disposition"):
                ...
                if result.get("holdout", {}).get("state_after") != entry.get("holdout_state"):
                    errors.append(f"{sid}: holdout state differs between index and result")
                if result.get("assurance", {}).get("status") != entry.get("assurance_status"):
    ...
    if directory_ids != known_ids:
        ...
        if missing_from_index:
            errors.append(f"index: unregistered study directories: {', '.join(missing_from_index)}")
        if missing_from_disk:
            errors.append(f"index: indexed study directories missing: {', '.join(missing_from_disk)}")
